package web.sistemaAluguelImovel.pagination;
...
public class PageWrapper<T> {
	...
	// LoggerFactory.getLogger(PageWrapper.class);
	...
	public PageWrapper(Page<T> pagina, HttpServletRequest request) {
		...
		String httpURL = requestURL.append(queryString != null ? "?" + queryString : "").toString().replaceAll("\\+",
				"%20");
		// logger.debug("PageWrapper criado para a requestURL: {}, queryString: {} e URL
		// final: {}", requestURL,
		// queryString, httpURL);
	...
	private String removeEmptyQueryParams(String queryString) {
		// logger.debug("Removendo os parâmetros vazios da queryString: {}",
		// queryString);
		String result = "";
		if (queryString != null) {
	...
	public String urlInvertendoDirecaoOrdem(String propriedade) {
		// logger.debug("Gerando uma nova URL com a ordem da propriedade {} invertida",
		// propriedade);
	...
	private void definirInicioFimPaginacao() {
		// logger.debug("Gerando a numeração da paginação para a Página {} e o máximo de
		// páginas na numeração {}", pagina,
		...
		// logger.debug("totalDePaginas: {}, paginaAtual: {}", totalDePaginas,
		// paginaAtual);
